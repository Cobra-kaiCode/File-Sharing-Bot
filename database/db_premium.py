...
async def add_premium(user_id, time_value, time_unit):
    ...
    # Calculate expiration time. If user already has active premium, extend from current expiry.
    now = datetime.now(ist)
    base_time = now
    existing = await collection.find_one({"user_id": user_id})
    if existing and existing.get("expiration_timestamp"):
        try:
            old_expiry = datetime.fromisoformat(existing["expiration_timestamp"]).astimezone(ist)
            if old_expiry > now:
                base_time = old_expiry
        except Exception:
            base_time = now

    if time_unit == 's':
        expiration_time = base_time + timedelta(seconds=time_value)
    elif time_unit == 'm':
        expiration_time = base_time + timedelta(minutes=time_value)
    elif time_unit == 'h':
        expiration_time = base_time + timedelta(hours=time_value)
    elif time_unit == 'd':
        expiration_time = base_time + timedelta(days=time_value)
    elif time_unit == 'y':
        expiration_time = base_time + timedelta(days=365 * time_value)
    else:
